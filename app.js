...
async function hydrateCloudAvatars(){var seen={},views=[];Object.keys(cloudProfiles).forEach(function(key){var view=cloudProfiles[key];if(view&&view.userId&&!seen[view.userId]){seen[view.userId]=true;views.push(view);}});await Promise.all(views.map(async function(view){if(!view.avatarPath)return;try{view.avatarUrl=await LvyueCloud.assetUrl('avatar',view.userId);}catch(error){view.avatarUrl='';}}));}
...
async function loadCloudData(){
  ...
  cloudUserId=own.user_id;cloudProfiles={};cloudCategories={};cloudFolders={};rememberCloudProfiles([own]);state=freshCloudState(own);
  state.folders=['全部相簿'].concat(folders.map(function(folder){cloudFolders[folder.name]=folder.id;return folder.name;}));
  state.friends=friends.map(function(friend){rememberCloudProfiles([{user_id:friend.user_id,travel_id:friend.travel_id,nickname:friend.nickname,avatar_path:friend.avatar_path}]);return {nickname:friend.nickname,id:friend.travel_id,avatar:(friend.nickname||friend.travel_id).slice(0,1),avatarUrl:'',color:'a2',remark:friend.display_name||friend.nickname,description:friend.description||'新添加的好友',friendshipId:friend.friendship_id,userId:friend.user_id};});
  state.friendRequests=requests.map(function(request){rememberCloudProfiles([{user_id:request.requester_id,travel_id:request.travel_id,nickname:request.nickname,avatar_path:request.avatar_path}]);return {requestId:request.id,fromId:request.travel_id,fromUserId:request.requester_id,nickname:request.nickname,avatar:(request.nickname||request.travel_id).slice(0,1),avatarUrl:'',createdAt:request.created_at};});
  media=[];
  for(var ai=0;ai<albumRows.length;ai++){
    var albumRow=albumRows[ai],albumDetail=await LvyueCloud.call('albums.get',{album_id:albumRow.id}),mediaRows=await LvyueCloud.call('media.list',{album_id:albumRow.id}),memberViews=albumDetail.members||[];
    cloudCategories[albumRow.id]={};(albumDetail.categories||[]).forEach(function(category){cloudCategories[albumRow.id][category.name]=category.id;});
    memberViews.forEach(function(member){rememberCloudProfiles([{user_id:member.user_id,travel_id:member.travel_id,nickname:member.nickname,avatar_path:member.avatar_path}]);});
    for(var mi=0;mi<mediaRows.length;mi++){
      var row=mediaRows[mi],src='',commentRows=[];try{src=await LvyueCloud.mediaUrl(row.id);}catch(error){}try{commentRows=await LvyueCloud.call('comments.list',{media_id:row.id});rememberCloudProfiles(commentRows);}catch(error){}
      function flattenReplies(parent){var result=[];commentRows.filter(function(item){return item.parent_id===parent.id;}).forEach(function(reply){result.push([reply.nickname,reply.body,parent.nickname,reply.travel_id,reply.id]);result=result.concat(flattenReplies(reply));});return result;}
      var mappedComments=commentRows.filter(function(item){return !item.parent_id;}).map(function(item){return [item.nickname,item.body,flattenReplies(item),item.travel_id,item.id];});
      media.push({id:row.id,album:row.album_id,src:src,storagePath:row.storage_path,people:(row.categories||[]).map(function(category){return category.name;}),by:row.uploader_nickname,byId:row.uploader_travel_id,uploaderUserId:row.uploader_id,date:row.captured_at||new Date(row.created_at).toLocaleDateString('zh-CN'),likes:Number(row.like_count||0),liked:Boolean(row.liked),shape:'medium',comments:mappedComments,video:row.media_type==='video',originalName:row.original_name,mimeType:row.mime_type,byteSize:row.byte_size});
    }
    var coverMedia=mediaRows.find(function(item){return item.storage_path===albumRow.cover_path;}),coverItem=coverMedia&&media.find(function(item){return item.id===coverMedia.id;}),coverUrl=coverItem&&coverItem.src||'';if(!coverUrl&&albumRow.cover_path)try{coverUrl=await LvyueCloud.assetUrl('cover',albumRow.id);}catch(error){}
    state.albums.push({id:albumRow.id,title:albumRow.title,place:albumRow.description||'',description:albumRow.description||'',cover:coverUrl,coverPath:albumRow.cover_path||'',folder:albumRow.folder_name||'',count:Number(albumRow.media_count||mediaRows.length),categories:['全部'].concat((albumDetail.categories||[]).map(function(item){return item.name;})),members:memberViews.map(function(item){return (item.nickname||item.travel_id).slice(0,1);}),memberIds:memberViews.map(function(item){return item.travel_id;}),ownerId:(memberViews.find(function(item){return item.role==='owner';})||{}).travel_id||'',ownerUserId:albumRow.creator_id});
  }
  state.albumReviews=[];
  for(var ri=0;ri<albumRows.length;ri++){
    var ownerAlbum=albumRows[ri];if(ownerAlbum.role!=='owner')continue;
    try{
      var reviewRows=await LvyueCloud.call('albums.reviews',{album_id:ownerAlbum.id});
      reviewRows.forEach(function(review){rememberCloudProfiles([{user_id:review.user_id,travel_id:review.travel_id,nickname:review.nickname,avatar_path:review.avatar_path}]);state.albumReviews.push({membershipId:review.id,albumId:ownerAlbum.id,albumTitle:ownerAlbum.title,userId:review.user_id,travelId:review.travel_id,nickname:review.nickname,createdAt:review.created_at});});
    }catch(error){}
  }
  rememberCloudProfiles(notices.filter(function(notice){return notice.actor_id&&notice.actor_travel_id;}).map(function(notice){return {user_id:notice.actor_id,travel_id:notice.actor_travel_id,nickname:notice.actor_nickname,avatar_path:notice.actor_avatar_path};}));
  rememberCloudProfiles(invites.filter(function(item){return item.inviter_id&&item.inviter_travel_id;}).map(function(item){return {user_id:item.inviter_id,travel_id:item.inviter_travel_id,nickname:item.inviter_nickname,avatar_path:item.inviter_avatar_path};}));
  await hydrateCloudAvatars();
  var ownView=cloudProfiles[String(cloudUserId).toLowerCase()];if(ownView)state.user.avatarUrl=ownView.avatarUrl;
  state.friends.forEach(function(friend){var view=cloudProfiles[String(friend.userId).toLowerCase()];if(view)friend.avatarUrl=view.avatarUrl;});
  state.friendRequests.forEach(function(request){var view=cloudProfiles[String(request.fromUserId).toLowerCase()];if(view)request.avatarUrl=view.avatarUrl;});
  state.notifications=notices.map(function(notice){return {id:notice.id,type:notice.type,text:notice.message,time:new Date(notice.created_at).toLocaleString('zh-CN'),read:Boolean(notice.read_at),actorId:notice.actor_travel_id||'',actorNickname:notice.actor_nickname||'',entityType:notice.entity_type,entityId:notice.entity_id};});
  state.albumInvites=invites.map(function(item){return {inviteId:item.id,albumId:item.album_id,albumTitle:item.title,fromId:item.inviter_travel_id||'',fromNickname:item.inviter_nickname||'好友',createdAt:item.created_at};});
}
...
function notify(text){toast=text;render();setTimeout(function(){toast='';render();},2200);}
...
function moveMedia(step){var items=viewerItems(),index=items.findIndex(function(item){return item.id===detail;});if(index<0||items.length<2)return;detail=items[(index+step+items.length)%items.length].id;render();}
...
async function createFolder(event){event.preventDefault();var name=String(new FormData(event.target).get('folder')).trim();if(state.folders.indexOf(name)>-1)return notify('这个分类已经存在');if(cloudMode){try{await LvyueCloud.rest('album_folders','',{method:'POST',headers:{Prefer:'return=minimal'},body:JSON.stringify({user_id:cloudUserId,name:name,position:state.folders.length-1})});await loadCloudData();closePopup();notify('已添加个人分类');}catch(error){notify(error.message||'添加分类失败');}return;}state.folders.push(name);save();closePopup();notify('已添加个人分类');}
async function renameFolder(event,indexValue){event.preventDefault();var index=Number(indexValue);if(index<=0||index>=state.folders.length)return;var name=String(new FormData(event.target).get('newName')).trim();if(!name)return;if(state.folders.some(function(item,itemIndex){return item===name&&itemIndex!==index;}))return notify('这个分类已经存在');var oldName=state.folders[index];if(cloudMode){try{await LvyueCloud.rest('album_folders','id=eq.'+cloudFolders[oldName],{method:'PATCH',headers:{Prefer:'return=minimal'},body:JSON.stringify({name:name})});if(state.folder===oldName)state.folder=name;await loadCloudData();notify('相簿分类名称已修改');}catch(error){notify(error.message||'修改失败');}return;}state.folders[index]=name;state.albums.forEach(function(item){if(item.folder===oldName)item.folder=name;});if(state.folder===oldName)state.folder=name;save();notify('相簿分类名称已修改');}
async function deleteFolder(indexValue){var index=Number(indexValue);if(index<=0||index>=state.folders.length)return;var removed=state.folders[index];if(cloudMode){try{await LvyueCloud.rest('album_folders','id=eq.'+cloudFolders[removed],{method:'DELETE',headers:{Prefer:'return=minimal'}});if(state.folder===removed)state.folder='全部相簿';await loadCloudData();notify('分类已删除，相簿仍保留在全部相簿');}catch(error){notify(error.message||'删除失败');}return;}state.folders.splice(index,1);state.albums.forEach(function(item){if(item.folder===removed)item.folder='';});if(state.folder===removed)state.folder='全部相簿';save();notify('分类已删除，相簿仍保留在全部相簿');}
async function createAlbum(event){event.preventDefault();var d=new FormData(event.target),description=d.get('place')+' · 刚刚创建';if(cloudMode){try{var newId=await LvyueCloud.rpc('create_album',{album_title:String(d.get('title')).trim(),album_description:description}),folderName=String(d.get('folder')||'');if(folderName&&cloudFolders[folderName])await LvyueCloud.rest('album_folder_items','',{method:'POST',headers:{Prefer:'return=minimal'},body:JSON.stringify({user_id:cloudUserId,album_id:newId,folder_id:cloudFolders[folderName]})});await loadCloudData();activeAlbum=newId;activePerson='全部';popup='';page='album';render();notify('新相簿已经创建');}catch(error){notify(error.message||'创建相簿失败');}return;}var id='album-'+Date.now();state.albums.unshift({id:id,title:d.get('title'),place:description,description:description,cover:'',folder:d.get('folder'),categories:['全部'],count:0,members:[state.user.avatar],memberIds:[currentLoginId],ownerId:currentLoginId});save();activeAlbum=id;activePerson='全部';popup='';page='album';render();notify('新相簿已经创建');}
...
async function updateAlbum(event){event.preventDefault();var d=new FormData(event.target),currentAlbum=album(),oldTitle=currentAlbum.title,newFolder=String(d.get('folder')||'');if(cloudMode){try{await LvyueCloud.call('albums.update',{album_id:currentAlbum.id,title:String(d.get('title')).trim(),description:String(d.get('description')||'').trim()});var selectedMedia=media.find(function(item){return item.album===currentAlbum.id&&item.src===pendingCover;});if(selectedMedia)await LvyueCloud.call('media.useAsCover',{media_id:selectedMedia.id});else if(/^data:/.test(pendingCover)){var coverBlob=await fetch(pendingCover).then(function(response){return response.blob();}),coverExt=(coverBlob.type.split('/')[1]||'jpg').replace('jpeg','jpg'),coverFile=new File([coverBlob],'cover-'+Date.now()+'.'+coverExt,{type:coverBlob.type||'image/jpeg'});await LvyueCloud.uploadFile('cover',currentAlbum.id,coverFile,[]);}await LvyueCloud.call('folders.assignAlbum',{album_id:currentAlbum.id,folder_id:newFolder&&cloudFolders[newFolder]?cloudFolders[newFolder]:null});await loadCloudData();closePopup();notify('相簿设置已保存');}catch(error){notify(error.message||'相簿设置保存失败');}return;}currentAlbum.title=d.get('title');currentAlbum.description=d.get('description');currentAlbum.cover=pendingCover;currentAlbum.folder=newFolder;save();notifyAlbumMembers(currentAlbum,'album',state.user.nickname+' 更新了相簿「'+oldTitle+'」的资料',{event:'album_updated'});closePopup();notify('相簿设置已保存');}
async function like(id){var m=media.filter(function(x){return x.id===id;})[0];if(!m)return;if(cloudMode){try{if(m.liked)await LvyueCloud.rest('media_likes','media_id=eq.'+id+'&user_id=eq.'+cloudUserId,{method:'DELETE',headers:{Prefer:'return=minimal'}});else await LvyueCloud.rest('media_likes','',{method:'POST',headers:{Prefer:'return=minimal'},body:JSON.stringify({media_id:id,user_id:cloudUserId})});m.liked=!m.liked;m.likes+=m.liked?1:-1;render();}catch(error){notify(error.message||'点赞失败');}return;}m.liked=!m.liked;m.likes+=m.liked?1:-1;if(m.liked&&m.byId)pushAccountMessage(m.byId,'like',state.user.nickname+' 赞了你的照片',{event:'media_liked',mediaId:m.id,albumId:m.album});render();}
async function comment(event,id){event.preventDefault();var text=String(new FormData(event.target).get('comment')).trim();if(!text)return;var item=media.filter(function(x){return x.id===id;})[0];if(cloudMode){try{await LvyueCloud.rest('comments','',{method:'POST',headers:{Prefer:'return=minimal'},body:JSON.stringify({media_id:id,user_id:cloudUserId,body:text})});await loadCloudData();render();}catch(error){notify(error.message||'评论发送失败');}return;}item.comments.push([state.user.nickname,text,[],currentLoginId]);if(item.byId)pushAccountMessage(item.byId,'comment',state.user.nickname+' 评论了你的照片：“'+text+'”',{event:'media_commented',mediaId:item.id,albumId:item.album});render();}
function openReply(key){replyingComment=replyingComment===key?'':key;render();setTimeout(function(){var input=document.querySelector('.reply-add input');if(input)input.focus();},0);}
async function replyComment(event,key){event.preventDefault();var parts=key.split(':');var item=media.find(function(x){return x.id===parts[0];});var index=Number(parts[1]);var text=String(new FormData(event.target).get('reply')).trim();if(!item||!item.comments[index]||!text)return;var parent=item.comments[index];parent[2]=parent[2]||[];var targetName=parent[0],targetId=parent[3]||item.byId,parentCommentId=parent[4];if(parts.length>2&&parent[2][Number(parts[2])]){var targetReply=parent[2][Number(parts[2])];targetName=targetReply[0];targetId=targetReply[3]||targetId;parentCommentId=targetReply[4]||parentCommentId;}if(cloudMode){try{await LvyueCloud.rest('comments','',{method:'POST',headers:{Prefer:'return=minimal'},body:JSON.stringify({media_id:item.id,user_id:cloudUserId,parent_id:parentCommentId,body:text})});replyingComment='';await loadCloudData();render();}catch(error){notify(error.message||'回复发送失败');}return;}parent[2].push([state.user.nickname,text,targetName,currentLoginId]);if(targetId)pushAccountMessage(targetId,'comment',state.user.nickname+' 回复了你的评论：“'+text+'”',{event:'comment_replied',mediaId:item.id,albumId:item.album});replyingComment='';render();}
...
async function upload(event){event.preventDefault();if(!selectedFiles.length)return notify('请先选择照片或视频');var d=new FormData(event.target),people=d.getAll('people'),currentAlbum=album(),uploadCount=selectedFiles.length;if(cloudMode){try{var categoryIds=people.map(function(name){return cloudCategories[currentAlbum.id]&&cloudCategories[currentAlbum.id][name];}).filter(Boolean);for(var index=0;index<selectedFiles.length;index++){notify('正在上传 '+(index+1)+' / '+uploadCount);await LvyueCloud.uploadFile('media',currentAlbum.id,selectedFiles[index],categoryIds,d.get('date')||null);}selectedFiles=[];picker.value='';await loadCloudData();closePopup();notify('已上传到共同相簿');}catch(error){notify(error.message||'上传失败，请检查网络后重试');}return;}selectedFiles.forEach(function(file,index){media.unshift({id:'upload-'+Date.now()+'-'+index,album:activeAlbum,src:URL.createObjectURL(file),people:people.length?people:[state.user.nickname],by:state.user.nickname,byId:currentLoginId,date:d.get('date')?String(d.get('date')).replaceAll('-','.'):'刚刚',likes:0,shape:index%2?'medium':'tall',comments:[],video:file.type.indexOf('video')===0});currentAlbum.count++;});save();notifyAlbumMembers(currentAlbum,'album',state.user.nickname+' 在相簿「'+currentAlbum.title+'」上传了 '+uploadCount+' 个新片段',{event:'media_uploaded'});selectedFiles=[];picker.value='';closePopup();notify('已上传到共同相簿');}
render();
bootCloudSession();
