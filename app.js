...
function patchAcceptedInvite(inviteId,joinedAlbum){var row=Array.from(document.querySelectorAll('[data-invite-id]')).find(function(item){return item.dataset.inviteId===String(inviteId);});if(row)row.remove();var list=document.querySelector('.request-list');if(list&&!list.children.length)list.innerHTML='<div class="message-empty">暂时没有新消息</div>';var badge=document.querySelector('.message-badge'),unread=(state.friendRequests||[]).length+(state.albumInvites||[]).length+(state.notifications||[]).filter(function(item){return !item.read;}).length;if(badge){if(unread)badge.textContent=unread;else badge.remove();}if(!joinedAlbum||page!=='albums'||state.folder!=='全部相簿')return;var exists=Array.from(document.querySelectorAll('[data-album-id]')).some(function(item){return item.dataset.albumId===String(joinedAlbum.id);});if(exists)return;var target=document.querySelector('.album-list .empty-add'),card=markupElement(albumCard(joinedAlbum),'.album-card');if(target&&card){target.parentNode.insertBefore(card,target);bindCommand(card,'onclick','click');}var count=document.querySelector('.hero-stat b');if(count)count.textContent=state.albums.length;}
function removeAlbumCardDom(albumId){Array.from(document.querySelectorAll('[data-album-id]')).forEach(function(item){if(item.dataset.albumId===String(albumId))item.remove();});var count=document.querySelector('.hero-stat b');if(count)count.textContent=state.albums.length;}
