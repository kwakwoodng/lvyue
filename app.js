...
async function hydrateCloudAvatars(){var seen={},views=[];Object.keys(cloudProfiles).forEach(function(key){var view=cloudProfiles[key];if(view&&view.userId&&!seen[view.userId]){seen[view.userId]=true;views.push(view);}});await Promise.all(views.map(async function(view){if(!view.avatarPath)return;try{view.avatarUrl=await LvyueCloud.assetUrl('avatar',view.userId);}catch(error){view.avatarUrl='';}}));}
  ...
  var cloudRefreshTimer=null,cloudRefreshPromise=null,cloudRefreshQueued=false;
  function queueCloudSync(delay){if(!cloudMode)return;clearTimeout(cloudRefreshTimer);cloudRefreshTimer=setTimeout(function(){syncCloudDataInBackground();},delay==null?250:delay);}
  function syncCloudDataInBackground(){
    if(!cloudMode)return Promise.resolve();
    if(cloudRefreshPromise){cloudRefreshQueued=true;return cloudRefreshPromise;}
    cloudRefreshPromise=loadCloudData().then(function(){render();}).catch(function(error){console.warn('cloud sync failed',error);}).then(function(){cloudRefreshPromise=null;if(cloudRefreshQueued){cloudRefreshQueued=false;queueCloudSync(100);}});
  ...
  state.friendRequests=requests.map(function(request){rememberCloudProfiles([{user_id:request.requester_id,travel_id:request.travel_id,nickname:request.nickname,avatar_path:request.avatar_path}]);return {requestId:request.id,fromId:request.travel_id,fromUserId:request.requester_id,nickname:request.nickname,avatar:(request.nickname||request.travel_id).slice(0,1),avatarUrl:'',createdAt:request.created_at};});
    var albumBundles=await mapWithConcurrency(albumRows,4,async function(albumRow){
      ...
      var mappedRows=await mapWithConcurrency(mediaRows,6,async function(row){
        var pair=await Promise.all([
          ...
          LvyueCloud.call('comments.list',{media_id:row.id}).catch(function(){return [];})
    ...
    media=[];albumBundles.forEach(function(bundle){cloudCategories[bundle.row.id]=bundle.categoryMap;rememberCloudProfiles(bundle.members.map(function(member){return {user_id:member.user_id,travel_id:member.travel_id,nickname:member.nickname,avatar_path:member.avatar_path};}));bundle.media.forEach(function(mapped){rememberCloudProfiles(mapped.profiles);media.push(mapped.item);});state.albums.push(bundle.album);});
    state.albumReviews=[];
...
async function createAlbum(event){event.preventDefault();var d=new FormData(event.target),title=String(d.get('title')).trim(),description=d.get('place')+' · 刚刚创建',folderName=String(d.get('folder')||'');if(cloudMode){try{var newId=await LvyueCloud.rpc('create_album',{album_title:title,album_description:description});if(folderName&&cloudFolders[folderName])await LvyueCloud.rest('album_folder_items','',{method:'POST',headers:{Prefer:'return=minimal'},body:JSON.stringify({user_id:cloudUserId,album_id:newId,folder_id:cloudFolders[folderName]})});state.albums.unshift({id:newId,title:title,place:description,description:description,cover:'',folder:folderName,categories:['全部'],count:0,members:[state.user.avatar],memberIds:[currentLoginId],ownerId:currentLoginId});activeAlbum=newId;activePerson='全部';popup='';page='album';render();notify('新相簿已经创建');queueCloudSync(0);}catch(error){notify(error.message||'创建相簿失败');}return;}var id='album-'+Date.now();state.albums.unshift({id:id,title:title,place:description,description:description,cover:'',folder:folderName,categories:['全部'],count:0,members:[state.user.avatar],memberIds:[currentLoginId],ownerId:currentLoginId});save();activeAlbum=id;activePerson='全部';popup='';page='album';render();notify('新相簿已经创建');}
...
async function comment(event,id){event.preventDefault();var text=String(new FormData(event.target).get('comment')).trim();if(!text)return;var item=media.filter(function(x){return x.id===id;})[0];if(!item)return;if(cloudMode){try{var created=await LvyueCloud.rest('comments','',{method:'POST',headers:{Prefer:'return=representation'},body:JSON.stringify({media_id:id,user_id:cloudUserId,body:text})});item.comments=item.comments||[];item.comments.push([state.user.nickname,text,[],currentLoginId,created&&created[0]&&created[0].id]);render();queueCloudSync(0);}catch(error){notify(error.message||'评论发送失败');}return;}item.comments.push([state.user.nickname,text,[],currentLoginId]);if(item.byId)pushAccountMessage(item.byId,'comment',state.user.nickname+' 评论了你的照片：“'+text+'”',{event:'media_commented',mediaId:item.id,albumId:item.album});render();}
...
async function upload(event){event.preventDefault();if(!selectedFiles.length)return notify('请先选择照片或视频');var d=new FormData(event.target),people=d.getAll('people'),currentAlbum=album(),uploadCount=selectedFiles.length;if(cloudMode){try{var filesToUpload=selectedFiles.slice(),categoryIds=people.map(function(name){return cloudCategories[currentAlbum.id]&&cloudCategories[currentAlbum.id][name];}).filter(Boolean);for(var index=0;index<filesToUpload.length;index++){notify('正在上传 '+(index+1)+' / '+uploadCount);await LvyueCloud.uploadFile('media',currentAlbum.id,filesToUpload[index],categoryIds,d.get('date')||null);}selectedFiles=[];picker.value='';closePopup();render();notify('已上传到共同相簿');queueCloudSync(0);}catch(error){notify(error.message||'上传失败，请检查网络后重试');}return;}selectedFiles.forEach(function(file,index){media.unshift({id:'upload-'+Date.now()+'-'+index,album:activeAlbum,src:URL.createObjectURL(file),people:people.length?people:[state.user.nickname],by:state.user.nickname,byId:currentLoginId,date:d.get('date')?String(d.get('date')).replaceAll('-','.'):'刚刚',likes:0,shape:index%2?'medium':'tall',comments:[],video:file.type.indexOf('video')===0});currentAlbum.count++;});save();notifyAlbumMembers(currentAlbum,'album',state.user.nickname+' 在相簿「'+currentAlbum.title+'」上传了 '+uploadCount+' 个新片段',{event:'media_uploaded'});selectedFiles=[];picker.value='';closePopup();notify('已上传到共同相簿');}
render();
bootCloudSession();
